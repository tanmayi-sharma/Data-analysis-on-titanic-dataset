...
str(TrainTitanic)
#now TrainTitanic dataframe contains only 7 variables- Survived,Pclass,Sex,Age,SibSp,Parch,Fare
...
# 4.Each significance level is associated to a symbol : p-values(0, 0.001, 0.01, 0.05, 0.1, 1) <=> symbols("***", "**", "*", ".", " ")


#STEP 3: BUILDING A LOGISTIC REGRESSION MODEL

#Building a model taking all variables intlo consideration

#Model1
LogModel1 = glm(Survived~., data=TrainTitanic, family= binomial)
summary(LogModel1)

#clearly, we can see in the chart.Corrplot() that Pclass and Fare are Strongly related.But, we dont take this into consideration for fitting our model because, our objective is to find which and what type of people "Survived" andwhat has affected their Survival.
...
#Clearly, the AIC of LogModel5 is the least.So, create a stepwise variable selection model for that.
...
#since the AIC of Full model and stepwise model are same, full model is necessary for making proper predictions.

#STEP 5:COMPUTING THE ACCURACY OF FULL MODEL
